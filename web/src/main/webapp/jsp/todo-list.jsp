...
<body>
	<section id="todoapp">
		<header id="header">
			<h1>Todos</h1>
			<form action="<c:url value="insert"/>" method="POST" accept-charset="utf-8">
				<input type="hidden" name="filter" value="${filter}" />
				<input id="new-todo" name="name" placeholder="Who needs to be done?" autofocus>
			</form>
		</header>
		<section id="main">
			...
			<ul id="todo-list">
				<c:forEach var="toDoItem" items="${toDoItems}" varStatus="status">
					<li id="toDoItem_${status.count}" class="<c:if test="${toDoItem.completed}">completed</c:if>" ondblclick="javascript:document.getElementById('toDoItem_${status.count}').className += ' editing';document.getElementById('toDoItemName_${status.count}').focus();">
						<div class="view">
							<form action="<c:url value="toggleStatus"/>" method="POST" id="toggleForm_${status.count}">
